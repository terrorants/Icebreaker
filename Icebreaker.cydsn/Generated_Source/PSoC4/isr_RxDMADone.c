...
{
    #ifdef isr_RxDMADone_INTERRUPT_INTERRUPT_CALLBACK
        isr_RxDMADone_Interrupt_InterruptCallback();
    #endif /* isr_RxDMADone_INTERRUPT_INTERRUPT_CALLBACK */ 
...
{
    CyRamVectors[CYINT_IRQ_BASE + isr_RxDMADone__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + isr_RxDMADone__INTC_NUMBER];
}
...
*
*   Note calling isr_RxDMADone_Start or isr_RxDMADone_StartEx will 
*   override any effect this API would have had. This API should only be called
*   after isr_RxDMADone_Start or isr_RxDMADone_StartEx has been called. 
...
void isr_RxDMADone_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((isr_RxDMADone__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *isr_RxDMADone_INTC_PRIOR = (*isr_RxDMADone_INTC_PRIOR & (uint32)(~isr_RxDMADone__INTC_PRIOR_MASK)) |
...
uint8 isr_RxDMADone_GetPriority(void)
{
    ...
    priority = (*isr_RxDMADone_INTC_PRIOR & isr_RxDMADone__INTC_PRIOR_MASK) >> priorityOffset;
